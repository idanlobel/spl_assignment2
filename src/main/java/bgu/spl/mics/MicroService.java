...
public abstract class MicroService implements Runnable {
    ...
    protected Map<Class<? extends Message>, Callback<?>> messages;
    protected Future<Model> result_of_proccess;
    protected static MessageBusImpl messageBus=MessageBusImpl.getInstance();
    /**
    ...
    public MicroService(String name) {
        this.name = name;
        this.messages=new HashMap<Class<? extends Message>, Callback<?>>();
        result_of_proccess=new Future<Model>();
    }
                ...
                try {
                    ...
                    if (message != null) {
                        ...
                        if (messages.containsKey(message.getClass())) {
                            Callback callback = messages.get(message);
...
//                }
    ...
    public Message getFirstMessage(){
        return messageBus.getMessagesHashMap().get(this).peek();
    }

    public Map<Class<? extends Message>, Callback<?>>  get_messages()
    {
        return this.messages;
    }
    public List<Event> getGoodResults(){
        List<Event> listOfGoodModelResults=new LinkedList<>();
        for(Event e:messageBus.getEventFutureHashMape().keySet()){
            if(messageBus.getEventFutureHashMape().get(e).get().equals("good"))
                listOfGoodModelResults.add(e);
        }
        return listOfGoodModelResults;
    }

    public List<Event> getGoodResultsByStudent(StudentService s){
        List<Event> goodResultsModels=new LinkedList<>();
        for(Message m:messageBus.getMessagesHashMap().get(s)){
            if(messageBus.getEventFutureHashMape().containsKey(m))
                if(messageBus.getEventFutureHashMape().get(m).get()!=null)
                    if(messageBus.getEventFutureHashMape().get(m).get()=="good")
                        goodResultsModels.add((Event)m);
        }
        return goodResultsModels;
    }
    public Queue<Class<? extends Broadcast>> get_broadcasts()
    {
        return messageBus.getSubscribeBroadCasts().get(this);
    }

    public ConcurrentHashMap<MicroService, BlockingQueue<Message>> getMessagesHashMap(){
        return messageBus.getMessagesHashMap();
    }
    public int getTickTime(){
        return messageBus.getTickTime();
    }
}