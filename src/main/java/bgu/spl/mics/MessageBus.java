...
public interface MessageBus {
     ...
     * @param type 	The type to subscribe to.
     * @param m    	The subscribing micro-service.
     * subscribeEvents.get(m).at(subscribeEvents.g e t(m).size()-1)==type
     * @PRE:subscribeBroadcats.get(m).contains(type)==false
     * @POST:subscribeBroadcats.get(m).contains(type)==true&&
     * subscribeBroadcats.get(m).at(subscribeBroadcats.g e t(m).size()-1)==type
     */
     ...
     * @POST:(for (MicroService microServ ice: subscribeEvents.key Set())
     * 			if(subscribeEvents.get(microService).contains(e.getClass()))
